...
iOS 26's `tabViewBottomAccessory` is the slot the Music app uses for its mini player — a glass capsule sitting on the tab bar, sharing its safe area. We enable it only on the planner tab and moved the existing controls in unchanged. The system draws the glass, so the buttons are just glyphs.
