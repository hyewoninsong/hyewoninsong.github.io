...
summary: "The daily planner's Home tab is gone, folded into the Todos tab. The app now opens on the planner, and each todo row carries its own contribution graph."
---
...
So Home is gone and its cards are now the top section of the Todos list. The per-todo card got absorbed into the rows themselves: every active row carries a strip graph, and the chevron on the right expands that todo's full graph underneath.

![Summary and list in one scroll. The small strip on each row is the last 14 days, total time underneath; the chevron expands that todo's full graph](/blog/planner-two-tabs/todos-tab.png)
...
Now the tiles, the granularity picker and the full graph stay visible, and the other four sit behind **More stats**, collapsed by default and remembered once you open it.

![More stats reveals completion rate, top todos, the heatmap and stale todos](/blog/planner-two-tabs/more-stats.png)
...
## One more thing: expanding the last row drew off-screen

Expanding the graph on the bottom row drew it under the tab bar. The expand action now carries a `ScrollViewReader` scroll that brings that row to the top, inside the same animation, so it reads as one motion.
